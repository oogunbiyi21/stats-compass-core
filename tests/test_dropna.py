...
class TestDropNA:
    ...
    def test_drop_rows_with_any_na(self) -> None:
        ...
        df = pd.DataFrame(
            {"A": [1, 2, None, 4], "B": [5, None, 7, 8], "C": [9, 10, 11, 12]}
        )
    ...
    def test_drop_rows_with_all_na(self) -> None:
        ...
        df = pd.DataFrame(
            {"A": [1, None, None, 4], "B": [5, None, None, 8], "C": [9, None, None, 12]}
        )

        params = DropNAInput(axis=0, how="all")
        result = drop_na(df, params)

        assert len(result) == 2  # Rows 0 and 3 kept (rows 1 and 2 have all NAs)
        assert result["A"].tolist() == [1.0, 4.0]
        assert result["C"].tolist() == [9, 12]
    ...
    def test_drop_with_threshold(self) -> None:
        ...
        df = pd.DataFrame(
            {"A": [1, 2, None, 4], "B": [5, None, None, 8], "C": [9, 10, None, 12]}
        )
    ...
    def test_drop_with_subset(self) -> None:
        ...
        df = pd.DataFrame(
            {"A": [1, 2, None, 4], "B": [5, 6, 7, 8], "C": [9, None, 11, 12]}
        )
